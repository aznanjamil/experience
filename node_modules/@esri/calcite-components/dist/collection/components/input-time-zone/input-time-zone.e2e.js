    ...
    describe("mode", () => {
        describe("offset (default)", () => {
            ...
            it("allows users to preselect a time zone offset", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await addTimeZoneNamePolyfill(html `<calcite-input-time-zone value="${testTimeZoneItems[1].offset}"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
        ...
        describe("name", () => {
            ...
            it("allows users to preselect a time zone by name", async () => {
                const page = await newE2EPage();
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(await addTimeZoneNamePolyfill(html `<calcite-input-time-zone mode="name" value="${testTimeZoneItems[1].name}"></calcite-input-time-zone>`));
                const input = await page.find("calcite-input-time-zone");
    ...
    describe("clearable", () => {
        ...
        describe("clearing by value", () => {
            ...
            beforeEach(async () => {
                ...
                await page.emulateTimezone(testTimeZoneItems[0].name);
                await page.setContent(addTimeZoneNamePolyfill(html ` <calcite-input-time-zone value="${testTimeZoneItems[1].offset}" clearable></calcite-input-time-zone>`));
        ...
        it("can be cleared on initialization when clearable is enabled", async () => {
            const page = await newE2EPage();
            await page.emulateTimezone(testTimeZoneItems[0].name);
            await page.setContent(addTimeZoneNamePolyfill(html `<calcite-input-time-zone value="" clearable></calcite-input-time-zone>`));
            const input = await page.find("calcite-input-time-zone");
            expect(await input.getProperty("value")).toBe("");
        });
        it("selects user time zone value when value is not set and clearable is enabled", async () => {
            const page = await newE2EPage();
            await page.emulateTimezone(testTimeZoneItems[0].name);
            await page.setContent(addTimeZoneNamePolyfill(html `<calcite-input-time-zone clearable></calcite-input-time-zone>`));
            const input = await page.find("calcite-input-time-zone");
            expect(await input.getProperty("value")).toBe(`${testTimeZoneItems[0].offset}`);
        });
    });
    describe("selection of subsequent items with the same offset", () => {
        ...
        testCases.forEach(({ name, initialTimeZoneItem }) => {
            it(`${name}`, async () => {
                ...
                await page.setContent(addTimeZoneNamePolyfill(html `<calcite-input-time-zone value="${initialTimeZoneItem.offset}"></calcite-input-time-zone> `));
                const input = await page.find("calcite-input-time-zone");
                await input.click();
                await page.waitForChanges();
                await input.type("(GMT-6)");
                await page.waitForChanges();
                const sharedOffsetTimeZoneItems = await page.findAll("calcite-input-time-zone >>> calcite-combobox-item:not([hidden])");
