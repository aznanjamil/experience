...
describe("calcite-slider", () => {
    ...
    describe("when range thumbs overlap", () => {
      ...
      snap
      style="width:${sliderWidthFor1To1PixelValueTrack}"`;
        async function assertValuesUnchanged(minMaxValue) {
    ...
    describe("snap + step interaction", () => {
        ...
        it("snaps to max limit beyond upper bound", async () => {
            await page.setContent(html `<calcite-slider max="10.5" min="0" snap step="1" ticks="1" value="10.5"></calcite-slider>`);
            const slider = await page.find("calcite-slider");
        ...
        it("snaps to max limit at upper bound", async () => {
            await page.setContent(html `<calcite-slider max="10.4" min="0" snap step="1" ticks="1" value="10.4"></calcite-slider>`);
            const slider = await page.find("calcite-slider");
            expect(await slider.getProperty("value")).toBe(10);
            await dragThumbToMax();
            expect(await slider.getProperty("value")).toBe(10);
    ...
    describe("labelFormatter", () => {
        ...
        describe("single value", () => {
            it("allows formatting of the handle and ticks", async () => {
                const page = await newE2EPage();
                await page.setContent(html ` <calcite-slider min="0" max="100" value="50" label-handles label-ticks ticks="25"></calcite-slider>`);
                await page.$eval("calcite-slider", (slider) => (slider.labelFormatter = (value, type) => {
                    if (type === "value") {
                        return `${value}%`;
                    }
                    if (type === "tick") {
            ...
            group-separator
          ></calcite-slider>`);
                await page.$eval("calcite-slider", (slider) => (slider.labelFormatter = (value, type, defaultFormatter) => {
                    if (type === "value") {
        ...
        describe("min/max value", () => {
          ...
          ></calcite-slider>`);
            ...
            group-separator
          ></calcite-slider>`);
                await page.$eval("calcite-slider", (slider) => (slider.labelFormatter = (value, type, defaultFormatter) => type === "min"
                    ? // default formatting
                        undefined
