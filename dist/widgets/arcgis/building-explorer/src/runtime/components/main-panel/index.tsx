...
export const MainPanel = React.memo((props: Props) => {
  ...
  // funs refs
  const unpdateWidgetRef = React.useRef<(domRef: HTMLDivElement, reason: string) => __esri.BuildingExplorer>()
  unpdateWidgetRef.current = updateBuildingExplorerWidget
  ...
  React.useEffect(() => {
    unpdateWidgetRef.current(createContainerDom(), UpdateReason.SwitchMap) //DO NOT clear filters
  ...
  React.useEffect(() => {
    if (!jimuMapViewChangedFlagRef.current) {
      ...
      unpdateWidgetRef.current(createContainerDom(), UpdateReason.Init)
    }
    ...
    <div className={'d-flex flex-column w-100 p-2 '} css={getStyle(theme)}>
      ...
      {<div className={'w-100 ' + (!isNoBuildingLayersStates ? 'd-flex' : 'd-none')}>
        <LayerSelectors
          ...
          onLayerSelecteChanged={setSelectedLayerViewIdsState}
        ></LayerSelectors>
      </div>}
      {/* no layers tips */}
