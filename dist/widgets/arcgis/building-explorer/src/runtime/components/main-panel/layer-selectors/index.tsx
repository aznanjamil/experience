...
export interface Props {
  ...
  onLayerSelecteChanged: (layers: string[]) => void
}
...
export const LayerSelectors = React.memo((props: Props) => {
  ...
  // layers selectors chagne
  const { onLayerSelecteChanged } = props
  React.useEffect(() => {
    _zoomToLayers(_selectedLayerViewsStates)
    onLayerSelecteChanged(_selectedLayerViewsStates)
  }, [_selectedLayerViewsStates, //changes
    _zoomToLayers, onLayerSelecteChanged])
  ...
  //   const selectedLayesrNum = (layerIds && layerIds.length)

  //   let selectedLabel = ''
  //   if (selectedLayesrNum <= 0) {
  //     selectedLabel = translate('selectLayerToExplore')
  //   } else if (selectedLayesrNum === 1) {
  ...
  //   } else {
  //     selectedLabel = translate('layersSelected', { number: selectedLayesrNum })
  //   }
