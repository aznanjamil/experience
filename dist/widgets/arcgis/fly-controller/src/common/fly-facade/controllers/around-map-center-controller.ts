...
export default class AroundMapCenterController extends BaseFlyController {
  ...
  initTimer (isImmediately: boolean, opts: flyOptions) {
    ...
    this.wattingTimer = setTimeout(() => {
      this.doFly(opts)
    }, time)
  }

  clearTimer = () => {
    clearTimeout(this.wattingTimer)
    this.wattingTimer = null
  }
  ...
  doFly = async (opts: flyOptions): Promise<any> => {
    ...
    } else if (this.wattingTimer && this.flyState === FlyState.RUNNING) {
