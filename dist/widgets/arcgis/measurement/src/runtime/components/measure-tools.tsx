...
const MeasureTools = React.forwardRef((props: MeasureToolsProps, ref: React.Ref<HTMLDivElement>): React.ReactElement => {
  const { config, context, activeButton, activeTool, translate, onSelectTool, onChangeUnit, onClear } = props
  const {
    enableDistance = true,
  ...
  React.useEffect(() => {
    if (activeButton === 'measureDistance') {
      setDistanceUnit(activeTool?.viewModel.unit as __esri.SystemOrLengthUnit || defaultDistanceUnit)
    }
    if (activeButton === 'measureArea') {
      setAreaUnit(activeTool?.viewModel.unit as __esri.SystemOrAreaUnit || defaultAreaUnit)
    }
  }, [activeButton, activeTool, defaultAreaUnit, defaultDistanceUnit])
