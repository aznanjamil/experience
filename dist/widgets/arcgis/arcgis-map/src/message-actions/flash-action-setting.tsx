...
class _FlashActionSetting extends React.PureComponent<ActionSettingProps<IMConfig> & ExtraProps, States> {
  ...
  /**
   * Get the webmap/webscene data source ids from appConfig.widgets.[wId].useDataSources
   * @param wId
   */
  getDsRootIdsByWidgetId = (wId: string): ImmutableArray<string> => {
  ...
  render () {
    ...
    // For flash message action, both trigger data and action data can only be a single data source, not multiple data sources.
