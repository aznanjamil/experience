...
import {
  ...
  getAppStore, MutableStoreManager, type ImmutableObject, type FeatureLayerDataSource, type UseDataSource,
  type ImmutableArray, type JSAPILayerMixin, css, jsx, lodash, serviceUrlUtils, observeStore, type ResourceSessions,
...
import {
  ...
  type UrlParamsMapOptions, getMapOptionsByUrlHashParams, getChangedViewpointByViewAndUrlHashParams
} from '../utils'
...
import { getValidPopupDockPosition } from '../../utils'
...
export type MapbaseView = (__esri.MapView | __esri.SceneView) & {
  ...
  isInCaching: boolean
  viewWhenInitialViewpoint: __esri.Viewpoint
...
export default class MapBase extends React.PureComponent<Props, State> {
    ...
    if (!calledWhenViewCreate) {
      if (config.popupDockPosition !== preConfig?.popupDockPosition) {
        ...
        if (this.view.popup) {
          ...
          this.updatePopupDockParams(this.view.popup, config.popupDockPosition)
        }
      }
    }
  ...
  updatePopupDockParams (popup: __esri.PopupProperties | __esri.Popup, popupDockPosition: string): void {
    if (!popup) {
      return
    }
  ...
  handleActionForPanToActionValue (mutableStateProps: ActionRelatedProps, mapBaseView: MapbaseView): void {
    ...
    } else if (panToType === 'go-to-map-initial-extent') {
      const initialViewpoint = this.getMapBaseInitViewPoint()
