...
import { loadArcGISJSAPIModules, type JimuMapView, LayerTypes, type JimuLayerView, type JimuFeatureLayerView, type JimuSceneLayerView, type JimuBuildingComponentSublayerView } from 'jimu-arcgis'
...
function queryFeatures (layerObject, querySQL: string, outFields: string[]) {
  return loadArcGISJSAPIModules([
    ...
    if (featureLayerView) {
      queryFeatures(featureLayerView.layer, querySQL, []).then(featureSet => {
        if (featureSet && featureSet.features && featureSet.features.length > 0) {
          ...
          startFlashFeatureLayer(jimuMapView.view, featureSet.features, symbol)
        }
      })
    } else {
      return null
...
function flashOnSceneLayer (jimuMapView: JimuMapView, querySQL: string, jimuLayerView: JimuSceneLayerView | JimuBuildingComponentSublayerView) {
  ...
  if (apiLayerView) {
    ...
    queryFeatures(layerToQuery, querySQL, [objectIdField]).then(featureSet => {
      if (featureSet && featureSet.features && featureSet.features.length > 0) {
        ...
        const flash = function () {
          if (i < maxFlashCount) {
            ...
            if (highlightColor) {
              ...
              sceneHightlightHandle = apiLayerView.highlight(objectIds)
            } else {
...
export async function flashFeaturesByQuery (jimuMapView: JimuMapView, layerDataSourceId: string, querySQL: string): Promise<void> {
  ...
  if (jimuLayerView && jimuLayerView.layer) {
    ...
    if ([LayerTypes.FeatureLayer, LayerTypes.ImageryLayer, LayerTypes.OrientedImageryLayer].includes(layerViewType)) {
      flashOnFeatureLayer(jimuMapView, querySQL, jimuLayerView as JimuFeatureLayerView)
    } else if ([LayerTypes.SceneLayer, LayerTypes.BuildingComponentSublayer].includes(layerViewType)) {
