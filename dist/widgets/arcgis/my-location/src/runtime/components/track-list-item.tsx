...
class TrackListItem extends React.PureComponent<Props & ExtraProps, State> {
  ...
  render () {
    ...
    return (
            <div className={'track-item' + (this.props.active ? ' active' : '')} key={t.Time} css={this.getStyle()} onClick={() => { this.handleSel(t) }} >
                <div className='head' >
                    <div className='title' >
                        <Button
                        ...
                        <div className='time'>{formatContent(this.props.intl, 'Time', t.Time)}</div>
                    </div>
