...
export class MarkPopper extends React.PureComponent<Props, States> {
  ...
  locateAndSetLayersConfig = () => {
    ...
    // location
    if (currentJimuMapView.view.type === '3d') {
      currentJimuMapView.view.goTo(this.Viewpoint.fromJSON(viewpoint))
    } else if (currentJimuMapView.view.type === '2d') {
      currentJimuMapView.view.goTo(this.Extent.fromJSON(extent))
    }
    // layers visibility
  ...
  getLayersConfig = (layersArray: __esri.Layer[]) => {
    ...
    const recursion = (array, config: LayersConfig) => {
      array.forEach(layer => {
        // layer's id could be a number, like 0 1 2 3
  ...
  addNewShowConfiger = async () => {
    ...
    const newBookmark = (snapShot: ResourceItemInfo) => {
      ...
      Object.keys(layersConfig).forEach((layerId) => {
        layersConfig[layerId].visibility = true
      })
      const bookmark: Bookmark = {
        id: this.bookmarkId,
