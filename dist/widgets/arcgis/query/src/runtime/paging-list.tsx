...
export function PagingList (props: PagingListProps) {
  ...
  const [defaultPopupTempalte, setDefaultPopupTempalte] = useState<any>()
  ...
  React.useEffect(() => {
    getPopupTemplate(outputDS, queryItem).then(rs => {
      ...
      if (rs.defaultPopupTemplate) {
        setDefaultPopupTempalte(rs.defaultPopupTemplate)
      }
    })
  }, [outputDS, queryItem])
      ...
      <div className='list_items mb-2 px-4' role='listbox'>
        {dataItems?.map((dataItem, x) => (
          <QueryResultItem
            ...
            defaultPopupTemplate={defaultPopupTempalte}
