...
import { type IconComponentProps, Loading, LoadingType, WidgetPlaceholder, utils, Alert, Label, Button, defaultMessages as jimuUIDefaultMessages, ConfirmDialog, Dropdown, DropdownButton, DropdownMenu, DropdownItem } from 'jimu-ui'
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  getAllFeaturesFromSelectedDs = () => {
    ...
    this.setState({
      ...
      getALLFeatures(this.props.selectedDataSource, null, true, this.state.jimuMapView.view.spatialReference, outFields).then((features) => {
  ...
  */
      ...
      featuresAndGroup.featuresGroup.forEach((group, groupIndex: number) => {
        ...
        if (group.subGroupInfo) {
          ...
          group.subGroupInfo.featuresSubGroup.forEach((subGroup, subGroupIndex: number) => {
            ...
            //sort features inside the subgroup using sort features by field cnfiguration
            if (!layerAnalysisInfo.sortFeaturesByDistance && layerAnalysisInfo.sortFeatures?.sortFeaturesByField &&
              layerAnalysisInfo.sortFeatures.sortFeaturesOrder === OrderRule.Desc) {
  ...
  getRecords = async (useDataSource: ImmutableObject<UseDataSource>) => {
    ...
    const promise = new Promise((resolve) => {
      ...
      getALLFeatures(ds, bufferGeometry, returnGeometry, this.state.jimuMapView.view.spatialReference, outFields).then((recordsList: DataRecord[]) => {
  ...
  downloadIndividualCsv = async (index: number, dsId: string, analysisId: string, analysisType: string) => {
    ...
    this.state.dataSetArray.forEach((dataSet) => {
      ...
      if (analysisType === AnalysisTypeName.Summary && dataSet.dataSource.id === currentDsId) {
        ...
        const records = this.featuresByDsId[dsId]
        if (dsId) {
          const dsManager = DataSourceManager.getInstance()
          const dataSource = dsManager?.getDataSource(dsId)
   ...
   */
  getFieldsToExport = (analysisId): string[] | ImmutableArray<string> => {
    ...
    configLayersInfo.forEach((layerInfo) => {
      if (layerInfo.analysisInfo.analysisId === analysisId) {
        ...
        configFieldsToExport = layerInfo.analysisInfo.fieldsToExport?.length > 0
          ? layerInfo.analysisInfo.fieldsToExport
          : getAllFieldsNames(layerInfo.useDataSource.dataSourceId)
      }
    })
  ...
  buildOutputDsResults = async (analysisLabel: string, outputDsId: string, featureRecords: DataRecord[], result: any) => {
    ...
    if (result.allowExport) {
      ...
      dataSetArr.push({
        ...
        fields: fieldsToExport as string[]
      })
