...
import { toCamelToolName, convertEsriMessageType, isEmptyDataItem, formatAnalysisEngineSuffix, canPerformStandardAnalysis, canPerformGeoAnalytics, canPerformRasterAnalysis, hasNAPrivilege, isStandardAnalysisServerAdvanced, type AnalysisHistoryItem, getRFxDefKeyFromRFT, type SerializedHistoryItem, isFeatureLayer } from '@arcgis/analysis-shared-utils'
import { AnalysisToolParamDataType, type AnalysisToolInfo, type GPFeatureRecordSetLayer, AnalysisEngine, type AnalysisToolItem, UserPrivileges, VisibilityModes, AnalysisType, type RFxTemplate, type AnalysisToolParam, AnalysisToolParamDirection } from '@arcgis/analysis-ui-schema'
 ...
 * Copy from @arcgis/analysis-shared-utils. beacuse if message.description is number, JSON.parse will not thow error, but 'cost' in parsedMessage will and the error is not catched, must catch the error here.
