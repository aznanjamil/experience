import { type AnalysisHistory, loadSavedResource, parseSerializedHistoryItem, InProgressStatuses, type AnalysisHistoryItem } from '@arcgis/analysis-shared-utils'
...
import { depthTraversalProcessingValue, destroyDataSources, filterHistoryItemMessages, getToolTypeByAnalysisType, isSameToolAsHistoryItem, resultValueIsFeatureLayer, resultValueIsFeatureSet, setValueToResultById } from './util'
...
export const loadAnalysisHistoryResourceItemsFromMap = async (jimuMapView: JimuMapView, portal: __esri.Portal) => {
  try {
    ...
    if (loadedResources !== undefined) {
      ...
      return allItems.filter((item) => item.analysisType !== AnalysisType.WebTool)
    }
  } catch (error) {
...
export const useParsedHistoryResourceFromMap = (config: IMConfig, oldHistoryFromMapList?: HistoryItemWithDs[]) => {
  ...
  const parsedHistoryResource = React.useMemo(() => {
    ...
    return historyResourceItemsFromMap.asMutable({ deep: true }).map((r) => {
      ...
      // since analysis component team did not handle with the histry saved in map in old version, must handle it here
        ...
        results: historyItem.results?.map((r, index) => {
          ...
          if (isCustomTool) {
            depthTraversalProcessingValue(result.value, `${index}`, (id: string, value: __esri.ParameterValue['value']) => {
              if (resultValueIsFeatureSet(value) || resultValueIsFeatureLayer(value)) {
