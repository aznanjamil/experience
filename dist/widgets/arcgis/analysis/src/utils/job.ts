import { buildProcessInfoReport, updateItemProperties, isEmptyDataItem, formatAnalysisEngineSuffix, getResultParams, generateRandomUniqueId } from '@arcgis/analysis-shared-utils'
...
export async function waitForJobCompeletionAndGetResults (
...
export async function completeHistoryInfo (historyItem: HistoryItemWithDs, importedFromMap: boolean, toolList: ImmutableArray<ToolConfig>, gpMessages: LocaleItem) {
  ...
  // if importedFromMap is true and history is from standard tool or ratser tool, there will be no toolId, so toolInfo will be undefined
  ...
  const { jobInfo, results } = await waitForJobCompeletionAndGetResults(historyItem.jobInfo, toolUrl, (jobInfo) => {}, gpMessages, resultParams, resultMapServerName)

  return { jobInfo, results }
}

/**
 * For sync jobs we dont have jobId as it is only for async jobs.
...
export async function executeJob (geoprocessor: __esri.geoprocessor, params: ExecuteProps, toolJson: AnalysisToolInfo, toolUrl: string, webToolServerUrl: string) {
  ...
  if (toolJson.executionType === 'esriExecutionTypeSynchronous') {
    ...
    try {
      const res: SynchronousJobExexuteResult = await geoprocessor.execute(toolUrl, jobParamsPayload, gpOptions)
  ...
  const jobInfo = await geoprocessor.submitJob(toolUrl, jobParamsPayload, gpOptions)
  notifyJobStatus(containerElement, {
    jobInfo,
    submissionData: {
      toolName: toolJson.name,
  ...
  const { results } = await waitForJobCompeletionAndGetResults(jobInfo, toolUrl, (jobInfo) => {
