...
import { type IMConfig, type SubmissionData, type ToolConfig, ToolType, type StandardToolConfig, type HistoryItemWithDs, type StandardToolOption, type AnalysisGPJobSubmited, type CustomToolOutput } from '../config'
...
const Widget = (props: Props) => {
  const { useMapWidgetIds, portalUrl, config, widgetId, theme, mutableStateProps } = props
  ...
  }, [canAccessAnalysisEngines, currentJimuMapView, currentToolInfo, getDisplayedToolName, historyListFromMap, mutableStateProps?.input, mutableStateProps?.toolId, onToolBack, openedTool, portal, signIn, theme, toolList.length, updateMutableStateInput, widgetConatiner, widgetId])
  ...
  useEffect(() => {
    const currentOpendToolId = (currentToolInfo || openedTool?.toolInfo)?.id
    if (mutableStateProps?.toolId) {
      if (currentOpendToolId !== mutableStateProps?.toolId) {
