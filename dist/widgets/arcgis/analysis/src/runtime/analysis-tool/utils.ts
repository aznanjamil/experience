...
export const useJobParamsFromOtherWidgets = (
  ...
  const [moduels, setModules] = React.useState<[typeof __esri.Layer, typeof __esri.FeatureLayer, typeof __esri.Graphic, typeof __esri.Field]>()
  React.useEffect(() => {
  ...
  const updateRealJobParamsAndTempLayers = async () => {
    ...
    const [Layer, FeatureLayer, Graphic, Field] = moduels
    ...
    if (paramNames.length) {
      ...
      // wait for analysis layer convert before update job params, otherwise the layers in jobParams cannot been recoganized
      lodash.defer(() => {
  ...
  React.useEffect(() => {
    if (!jobParamsFromOtherWidgets || !moduels) {
      return
    }
    updateRealJobParamsAndTempLayers()
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jobParamsFromOtherWidgets, moduels])
