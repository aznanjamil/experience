...
describe('convertInputGPValueParameters', () => {
  const mockMapView = new MockMapView()
  it('should change the array of array default value of GPValueTable to key-value array', () => {
    const convertedParams = convertInputGPValueParameters(
      [
        {
          name: 'Input_Features',
          dataType: 'GPValueTable',
    ...
    expect(convertedParams[0].defaultValue).toEqual([
      { 'feature layer column': 'polylines', 'feature set column': 'points' },
      { 'feature layer column': 'points', 'feature set column': 'polygonZ' }
    ])
  })
  it('should add mapView, mapLayers, user on parameterInfo of GPValueTable when parameterInfo type is layer input', () => {
    const convertedParams = convertInputGPValueParameters([
      {
        name: 'Input_Features',
        dataType: 'GPValueTable',
    ...
    ] as any[], mockMapView as any, [], undefined)
    expect(convertedParams[0].defaultValue).toEqual([{ 'feature class column': undefined, 'feature layer column': undefined, 'feature set column': undefined }])
    convertedParams[0].parameterInfos.forEach((p: any) => {
      expect(p.mapView).toEqual(mockMapView)
      expect(p.mapLayers).toEqual([])
      expect(p.user).toBeUndefined()
    })
  })
  it('should return the origin parameter if parameter type is not GPValueTable', () => {
    const param = {
      name: 'Query',
      dataType: 'GPString',
      displayName: 'Query',
    ...
    expect(convertInputGPValueParameters([param], mockMapView as any, [], undefined)).toEqual([param])
  })
})
