...
any> {
  ...
  async getGeoenrichmentServiceUrl () {
    ...
    if (this.portalOnlineGEProxy) {
      const url = this.geoenrichmentServiceUrl + '/Geoenrichment/countries?f=pjson&appID=esriexperiencebuilder' + (this.props.token ? '&token=' + this.props.token : '')
      await fetch(url)
        .then(async (response) => {
  ...
  async componentWillMount () {
    ...
    if (!this.state.countries) {
      ...
      const c: any = await getCountries(self.props.config.langCode, geUrl, self.props.token)
      if (c) {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    const { widgetMode, sourceCountry, selectedHierarchy, baSearchType, runReportOnClick, reportList, igBackgroundColor, selectedGeographyLevels, searchbarEnabled } = self.props.config
    ...
    const { stencilPropChangeConnected, selectedCountry, showInfographicModal, currentStep, workflowRuntimeSearchSelectedObject, workflowRuntimeSelectedFeatureObjectFirstTime, workflowRuntimeBuffer, mapSearchbarEnabled } = self.state
    ...
    if (this.props.user !== prevProps.user) {
      this.updateState('initializedGEUrl', false)
      this.getGeoenrichmentServiceUrl()
    }
    ...
    if (widgetMode === Mode.Workflow) {
      ...
      if (reportsElem) {
        // @ts-expect-error: Unreachable code error
        reportsElem.initialize(user.username, token, window.jimuConfig.hostEnv, sourceCountry, selectedHierarchy, langCode, JSON.stringify(reportList))
        return true
      }
      return false
    }
  ...
  render () {
    ...
    if (this.props.config.sourceCountry && this.props.config.selectedHierarchy && this.state.countries) {
      hierarchies = getValidHierarchies(this.props.config.sourceCountry, this.state.countries)
      if (hierarchies) {
        validHierarchy = hierarchies.find(o => o.ID === this.props.config.selectedHierarchy)
              ...
              <React.Fragment>
                {!showInfographicModal ? (
                  ...
                  <ArcgisInfographic
                    ...
                    selectedHierarchy={igData.selectedHierarchy}
