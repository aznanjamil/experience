...
export function AddSinglePointEvent (props: AddSinglePointEventProps) {
  ...
  React.useEffect(() => {
    if (selectedEvent) {
      const networkLayer = lrsLayers.find(item => item.serviceId === selectedEvent.eventInfo.parentNetworkId)
  ...
  React.useEffect(() => {
    if (isDefined(networkDataSourceFromDataAction)) {
      if (networkDataSourceFromDataAction.id !== selectedNetwork?.id) {
        ...
        const nds = networkDataSourceFromDataAction as FeatureLayerDataSourceImpl
        const layerId = nds.getLayerDefinition().id
        const eventLayer = lrsLayers.find((item) => item.eventInfo.parentNetworkId === layerId)
  ...
  const handleEventChanged = React.useCallback((value: string) => {
    ...
    if (eventLayer) {
      const networkLayer = lrsLayers.find(item => item.serviceId === eventLayer.eventInfo.parentNetworkId)
      if (networkLayer && networkLayer.layerType === LrsLayerType.network) {
        setSelectedNetwork(Immutable(networkLayer))
      }
