...
export const SplitEventAttributes = React.forwardRef((props: SplitEventAttributesProps, ref) => {
  ...
  React.useEffect(() => {
    onValidationChanged(!hasErrors && lockAquired)
  }, [hasErrors, lockAquired, onValidationChanged])
  ...
  const areNonLrsFieldsInvalid = (fieldInfos: FieldInfo[], hasAttributeError: boolean, isEvent1Attributes: boolean): boolean => {
    const fieldInfoCopy = cloneDeep(fieldInfos)
    let hasErrors = false

    // Check all fields for errors. We will display the last error message we encounter.
    fieldInfoCopy.forEach((field) => {
      let result
      if (field.hasDomain) {
      ...
      if (!hasAttributeError && result.hasError) {
        onUpdateToastMsgType('error')
        const fieldName = eventLayer.useFieldAlias ? field.alias : field.name
        onUpdateToastMsg(formatMessage(intl, 'attributeError', {
  ...
  const submitForm = async (locks?: LrsLocksInfo) => {
    ...
    const isInvalid = areNonLrsFieldsInvalid(fieldInfos, false, true)
    const isInvalid2 = areNonLrsFieldsInvalid(fieldInfos, isInvalid, false)
