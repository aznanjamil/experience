...
export function AddSingleLineEvent (props: AddSingleLineEventProps) {
  ...
  React.useEffect(() => {
    if (selectedEvent) {
      const networkLayer = lrsLayers.find(
        (item) => item.serviceId === selectedEvent.eventInfo.parentNetworkId
      )
      ...
      setTimeout(() => {
        if (isDefined(networkDataSourceFromDataAction)) {
          if (networkDataSourceFromDataAction.id !== selectedNetwork?.id) {
            ...
            const eventLayer = lrsLayers.find((item) => item.eventInfo.parentNetworkId === layerId && !item.eventInfo.isPointEvent)
  ...
  const handleEventChanged = React.useCallback(
    (value: string) => {
      ...
      if (eventLayer) {
        const networkLayer = lrsLayers.find(
          (item) => item.serviceId === eventLayer.eventInfo.parentNetworkId
        )
