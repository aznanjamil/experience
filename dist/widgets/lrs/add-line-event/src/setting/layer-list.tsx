...
export function LrsLayerList (props: Props) {
  const { widgetId, layers = [], mapWidgetIds, portalUrl, theme, onLayersAdded, onLayerRemoved, onClearAll, onLayerChanged, onLayerOrderChanged, onMapWidgetedChanged, onAttributeSetsChanged, intl } = props
          ...
          .then((result: any) => {
            ...
            if (type === 'esriLRSNetworkLayer' ||
              ...
              if (isNetwork && !result.isDerived) {
                ...
                eventInfo.parentNetworkId = result.parentNetwork.id as number
              } else if (isIntersection) {
                ...
                intersectionInfo.parentNetworkId = result.parentNetwork.id as number
              }
  ...
  const isParentNetworkAvailable = (index: number): boolean => {
    ...
    if (isDefined(eventLayer) && isDefined(eventLayer.eventInfo) && eventLayer.layerType === LrsLayerType.event) {
      return layers.findIndex(layer => layer.serviceId === eventLayer.eventInfo.parentNetworkId) > -1
    }
