...
import { GetUnits, isDefined, getRouteFromEndMeasures, type RouteInfo, getHoverGraphic, getSimplePointGraphic, QueryRouteMeasures, getDateWithTZOffset } from 'widgets/shared-code/lrs'
                      ...
                      .then(async (endpointMeasures) => {
                        ...
                        await QueryRouteMeasures(featureDS, networkInfo, [nearestPoint.coordinate],
                          ...
                          .then((measures) => {
                            ...
                            if (isDefined(attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name]) && defaultFromSelectedDate > attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name]) {
                              defaultFromSelectedDate = getDateWithTZOffset((attributes[layerDetails?.layer?.networkInfo?.fromDateFieldSchema?.name]), ds)
                            } else {
                              defaultFromSelectedDate.setHours(0, 0, 0, 0)
                            }

                            let defaultToSelectedDate = new Date(Date.now())
                            if (isDefined(attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name]) && defaultToSelectedDate > attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name]) {
                              defaultToSelectedDate = getDateWithTZOffset((attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name]), ds)
                            } else {
                              defaultToSelectedDate.setHours(0, 0, 0, 0)
                            }

                            if (!isDefined(attributes[layerDetails?.layer?.networkInfo?.toDateFieldSchema?.name])) {
