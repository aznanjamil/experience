...
import { type NetworkItem, type Style } from '../../config'
...
export const SearchByRouteResultItem = (props: ResultItemProps) => {
  const { widgetId, autoSelect, network, data, outputDS, inputDS, highlightGraphicLayer, flashGraphicsLayer, highlightStyle, jimuMapView, intl } = props
  ...
  const getIndividualPopupTemplate = () => {
    return getPopupTemplate(intl, data.feature as __esri.Graphic, outputDS as FeatureLayerDataSource, network)
  }
  ...
  const handleClickResultItem = React.useCallback(() => {
    ...
    if (nextSelectedDataItems.length > 0) {
