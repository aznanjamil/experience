...
import { executeMeasureToGeometry, queryRoutes, executeReferentToGeometry, executeGeometryToMeasure, queryRoutesByGeometry, queryRoutesByGeometryWithTolerance, queryRoutesByClosestResults, executeGeometryToMeasureWithTolerance } from '../utils/service-utils'
...
import { createLableExpression } from '../utils/utils'
...
export function SearchByRouteTask (props: SearchByRouteTaskProps) {
  ...
  const [selectedMethod, setSelectedMethod] = React.useState<SearchMethod>(selectedNetwork?.defaultMethod || SearchMethod.Measure)
  ...
  const handleMeasureFormSubmit = React.useCallback(async (routeQuery: RouteAndMeasureQuery) => {
    ...
    if (config.labelStyle.size > 0) {
      const layer = featureDS?.layer || (await featureDS.createJSAPILayerByDataSource()) as __esri.FeatureLayer
      featureDS.layer = layer

      const labelExpression = createLableExpression(selectedNetwork, routeQuery.isPoint)
      labelLayer = await createLabelLayer(featureDS, jimuMapView, labelExpression, labelFeatureId, config.labelStyle.color, config.labelStyle.size)
    }
    ...
    queryRoutes(originDS, selectedNetwork, routeQuery, jimuMapView)
      .then((routeRecords) => {
        ...
        executeMeasureToGeometry(widgetId, featureDS, selectedNetwork, routeRecords, routeQuery, labelLayer)
          .then((newRecords) => {
  ...
  const handleReferentFormSubmit = async (queryParams: ReferentQuery, objectIdFromDt: any[]) => {
    ...
    if (config.labelStyle.size > 0) {
      ...
      const labelExpression = createLableExpression(selectedNetwork, true)
      labelLayer = await createLabelLayer(featureDS, jimuMapView, labelExpression, labelFeatureId, config.labelStyle.color, config.labelStyle.size)
    }
    ...
    executeReferentToGeometry(widgetId, originDS, queryParams, selectedNetwork, selectedReferent,
      featureDS, count, objectIdFromDt, config, labelLayer)
      .then((newRecords) => {
        if (newRecords?.length > 0) {
    ...
    queryRoutesByGeometry(originDS, selectedNetwork, query)
      .then(async (routeRecords) => {
        if (routeRecords.length > 0) {
          ...
          executeGeometryToMeasure(widgetId, featureDS, selectedNetwork, routeRecords, query, labelLayer)
            .then((newRecords) => {
              featureDS.setRecords(newRecords)
              setResultCount(newRecords?.length)
            })
            ...
            .then(async (rteRecords) => {
              if (rteRecords.length > 0) {
                ...
                loadArcGISJSAPIModules(['esri/geometry/Point', 'esri/geometry/SpatialReference']).then(modules => {
                  ...
                  queryRoutesByClosestResults(originDS, selectedNetwork, featureDS, closestResults, closestRouteIds, allResultWkid, widgetId, labelLayer)
                    .then(({ newRecords }) => {
                      featureDS.setRecords(newRecords)
                      setResultCount(newRecords?.length)
                      if (newRecords?.length > 0) {
