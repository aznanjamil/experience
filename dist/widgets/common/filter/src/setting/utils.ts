...
export const getGroupName = (
  ...
  if (currentFI) {
    ...
    if (fItems) {
      const indexList = []
      fItems.filter(item => item.isGroup).forEach(item => {
        const groupPrefix = i18nMessage('groupName', { num: '' }).trim()
    ...
    label = i18nMessage('groupName', { num: index })
  }
  return label
}
...
export const getUseDataSourcesByDsAdded = (
  fItems: filterItemConfig[],
  addedUseDataSource: UseDataSource,
  useDataSources: ImmutableArray<UseDataSource>,
  // previousDsId?: string
  ...
  if (previousFItem) {
    if (previousFItem.isGroup) { // TODO: For group, it might need to keep fields when dss are changed.
  ...
  // add new ds id when it's not in useDss
  const isInUseDss = useDss.filter(useDs => addedUseDataSource.dataSourceId === useDs.dataSourceId).length > 0
  if (!isInUseDss) {
    useDss = useDss.concat(addedUseDataSource)
  }
  return useDss
}
