import { getAppStore, SessionManager, portalUrlUtils, esri, type QueriableDataSource, CONSTANTS } from 'jimu-core'
import { type FeatureLayerDataSource } from 'jimu-arcgis'
import { type HTTPMethods } from '@esri/arcgis-rest-request'
import { type ItemRelationshipType, type ICreateItemOptions } from '@esri/arcgis-rest-portal'
 ...
 */
export function getSurveyFromLayer (datasource: FeatureLayerDataSource) {
  resetSurveyServices()
  // todo: test the data source is added by single feature layer, or without service item id
  const serviceItemId = datasource.itemId || datasource.getMainDataSource()?.itemId
  if (!serviceItemId) {
 ...
 * @param dataSource
 */
export function extractParamFromDataSource (dataSource: FeatureLayerDataSource) {
  let url = ''
  let where = ''
  let objectIds = ''
  if (!dataSource) {
...
export function getSurveyIdFromSourceLayerOrFieldworkerView (serviceItemId: string): Promise<string> {
  ...
  return esri.restPortal.getItem(serviceItemId, {
    ...
    if (keywords.includes('Hosted Service') && keywords.includes('Survey123')) {
      ...
      if (targetItem) {
        const type = keywords.includes('Source') ? 'source' : 'fieldworker'
        surveyServices[type] = {
          itemId: item.id,
...
export function generateSampleReport (template: any, survey: any, dsParam: any, config: any, locale: string) {
  ...
  return new Promise((resolve, reject) => {
    fetch(url, {
      method: 'POST',
      headers: {
        'X-Survey123-Request-Source': 'website/Data',
...
function addCustomPrintingItem (surveyItemInfo: any, itemParams: any, currentUser: any, sectionTypeKeywords?: string[]): Promise<any> {
  ...
  return Promise.resolve(true)
    ...
    .then(() => {
      ...
      const params: ICreateItemOptions = Object.assign({
        item: {
...
export function shareCustomPrintingItem (surveyItemId: any, customPrintingItemId: string): Promise<boolean> {
  return getStakeholderViewFeatureService(surveyItemId)
    ...
    // .catch((err) => {
    //   if (err && err.code === 'customPrinting05') {
    //     return false;
    //   }
    //   throw err;
    // });
...
function getDefaultWordTemplate (featureLayerUrl, surveyItemId, contentType) {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.setRequestHeader('X-Survey123-Request-Source', 'website/Data')
