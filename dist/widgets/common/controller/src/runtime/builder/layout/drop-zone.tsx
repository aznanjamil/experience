/** @jsx jsx */
import { React, css, jsx, classNames, type IMSizeModeLayoutJson, type LayoutItemConstructorProps, type IMLayoutJson, hooks, appActions, getAppStore } from 'jimu-core'
...
export const DrapZone = React.forwardRef((props: DropZoneProps, ref) => {
  ...
  const onDrop = (draggingItem: LayoutItemConstructorProps, containerRect: Partial<DOMRectReadOnly>, itemRect: Partial<DOMRectReadOnly>) => {
    ...
    const layouts = getAppStore().getState().appConfig.layouts
    const { layoutId, layoutItemId } = draggingItem.layoutInfo || {}
    const inserWidgetId = layouts[layoutId]?.content?.[layoutItemId]?.widgetId
    inserWidgetId && getAppStore().dispatch(appActions.closeWidget(inserWidgetId))
