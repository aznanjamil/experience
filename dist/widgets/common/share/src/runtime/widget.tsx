...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & ExtraProps, State> {
  ...
  private readonly _cached = {
    currentUrl: '',
    shortUrl: '', // use longUrl to exchanged from the backend, '' means havn't fetch shortUrl
    enableShortUrl: true, // ref for remember whether is isShortUrl
    _urlAttachdeOrg: '', // just for testing
    _debug: false
  }
  ...
  updateUrls = (options?: { enableShortUrl?: boolean, urlExcludedUrlParams?: string }): void => {
    ...
    this._cached._urlAttachdeOrg = href
    ...
    if (this._cached._debug) {
      ...
      console.log('isInludeUrlParams==>' + !excludedUrlParamsFlag)
      console.log('_cached.currentUrl==>' + this._cached.currentUrl)
    }
  }

  hanldError = (type?: string): void => {
    if (!type || type === 'clear') {
      this.setState({ errorInfo: null })
    }
  }

  // complete url(including ?org=<urlkey>), but may not include UrlParams
  ...
  render () {
    ...
    const isHideMainContentInPopuoMode = [ItemsName.Embed, ItemsName.QRcode, ItemsName.Sharelink].includes(shownItem)
    ...
    const mainContent = <React.Fragment>
      ...
      {(uiMode === UiMode.Popup) &&
        <div className={'popup-mode-content ' + (isHideMainContentInPopuoMode ? 'd-none' : '')}>
          <ShareLink
            sharedUrl={sharedUrl}
    ...
    return (
      <div css={getStyle(this.props.theme)} data-testid='share-widget'>
        <React.Fragment>
          {/* 1.popup mode */}
          {(uiMode === UiMode.Popup) && !this.state.isInController &&
