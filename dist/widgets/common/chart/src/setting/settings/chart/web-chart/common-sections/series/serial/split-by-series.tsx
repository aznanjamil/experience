...
const SplitBySeries = (props: SplitBySeriesSettingProps): React.ReactElement => {
  ...
  const spliyByValues = React.useMemo(() => {
  ...
  const validity = React.useCallback(
    (value): ValidityResult => {
      ...
      const existed = spliyByValues.includes(value)
      if (existed) {
        return {
          valid: false,
  ...
  return (<Root className={classNames('split-by-series-setting-series', className)} role='group' aria-label={translate('sliceColor')}>
        ...
        {propSeries?.map((serie, index) => {
          ...
          return (
            <SeriesItem
              ...
              value={serie}
              undefinedItem={serie.id === SplitByOtherSeriesValue}
    ...
    <div className='footer'>
      <div className='px-4'>
        <SeriesLoader className='my-2' dataSourceId={dataSourceId} splitByField={splitByField} values={spliyByValues} onChange={handleSplitByValuesChange}></SeriesLoader>
