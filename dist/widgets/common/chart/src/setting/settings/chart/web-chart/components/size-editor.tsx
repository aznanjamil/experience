...
import { defaultMessages, Dropdown, DropdownButton, DropdownItem, DropdownMenu, defaultMessages as jimuiDefaultMessage, NumericInput } from 'jimu-ui'
        ...
        <DropdownMenu className='p-0' css={dropdownMenuStyle}>
          {SizeModes.map((item, index) => (
            <DropdownItem active={item === mode} aria-label={translate(item)} key={index} onClick={() => { handleModeChange(item) }}>
              <div className='d-flex alitn-items-center justify-content-end w-100'>
