...
export function getInsideLayersFromWebmap (dataSources: { [dsId: string]: DataSource }, layerList) {
  ...
  if (mapDs?.type === ArcGISDataSourceTypes.WebMap) {
    ...
    mapDs.getAllChildDataSources().forEach(layer => {
      ...
      if (
        ((layer.type === DataSourceTypes.MapService || layer.type === DataSourceTypes.ImageryLayer || layer.type === DataSourceTypes.ImageryTileLayer) && (layer as MapServiceDataSource).supportTime()) ||
        (
...
export function isSingleLayer (dsType: string): boolean {
  return dsType === DataSourceTypes.FeatureLayer || dsType === DataSourceTypes.ImageryLayer || dsType === DataSourceTypes.ImageryTileLayer
}
