...
interface SelectHeaderProps {
  ...
  setSelectByLoactionVisible: React.Dispatch<React.SetStateAction<boolean>>
...
/**
 * SelectHeader includes two pars: select tools and clear button.
 */
export default function SelectHeader (props: SelectHeaderProps) {
  ...
  const isLoadingApiModuelsRef = React.useRef<boolean>(false)
  ...
  const enableAttributeSelection = config.mapInfo?.enableAttributeSelection || false
  ...
  // load api modules
  React.useEffect(() => {
    if (needInteractiveAPIModules && !apiModules && !isLoadingApiModuelsRef.current) {
      isLoadingApiModuelsRef.current = true
  ...
  }, [apiModules, isLoadingApiModuelsRef, needInteractiveAPIModules])
  ...
  React.useEffect(() => {
    if (selectedToolType && !allSelectToolTypes.includes(selectedToolType)) {
      ...
      // user turns on 'Select by data' option for 'Select by attirbutes'
  ...
  const isSelectionModeEnabled = !!(useMap && !enableAttributeSelection && interactiveTool)
  ...
  const onSketchViewModelComplete = React.useCallback((apiView: __esri.MapView | __esri.SceneView, graphic: __esri.Graphic, selectionMode: DataSourceSelectionMode) => {
    ...
    try {
      if (geometry.type === 'point' || geometry.type === 'polyline') {
        ...
        // To sovle the above issues, we need to buffer the geometry if it is point or polyline.
  ...
  React.useEffect(() => {
        ...
        if (preActiveInteractiveTool === activeInteractiveTool) {
          ...
          if (newView) {
            if (newView === sketchView) {
              ...
              // need to destroy current sketchViewModel and recreate it, dont't change sketchViewModel.view,
    ...
    if (apiModules && loadedActiveView && activeInteractiveTool && !newSketchViewModel) {
      ...
      // When draw retangle, it always draw square if shift key is pressed, need to override this behaviour.
      // Bind key-down/key-up envents on view before we create SketchViewModel,
  ...
  // auto update isSelectByLoactionVisible
  React.useEffect(() => {
    const isSelectByLoactionVisible = isSelectedBtnActive && selectedToolType === SELECT_BY_DATA
    setSelectByLoactionVisible(isSelectByLoactionVisible)
  }, [isSelectedBtnActive, selectedToolType, setSelectByLoactionVisible])
...
${newSelectionTip}`
  ...
  const onToogleToolBtnActive = React.useCallback(() => {
