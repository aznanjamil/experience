 ...
 * This component provides the following methods to udpate selection:
 ...
 *  1. If not related to a jimuLayerViewId, then we need to create data source immediatelly because we need to get the label from data source.
 *  2. If realted to a jimuLayerViewId, then we need to wait jimuLayerView loaded immediatelly because we need to get the label from jimuLayerView.
 *     And we don't need to create data source for it immediatelly, we only create data source for it when necessary.
 *
 ...
 *   4. Need to show SqlExresssionRuntime or show CustomSqlBuilder.
 */
export default function DataSourceListItem (props: DataSourceListItemProps): React.ReactElement {
  ...
  const isGenerated = itemRuntimeInfo.isGenerated
  const shouldRenderCustomSqlIcon = isGenerated
  ...
  const shouldRenderSqlExpressionRuntime = React.useMemo(() => {
    return shouldShowSqlExpressionRuntime(isGenerated, configSqlExpression)
  }, [isGenerated, configSqlExpression])

  // const translate = hooks.useTranslation(jimuUIMessages, defaultMessages)
  const finalSqlHint = React.useMemo(() => {
    const hint = isGenerated ? translate('customSQL') : configSqlHint
    return hint || ''
  }, [configSqlHint, isGenerated, translate])
  ...
  //  5. Need to show SqlExresssionRuntime or show CustomSqlBuilder.
  const isFinalNeedDataSource = React.useMemo(() => {
    ...
    const isNeedToShowSqlExresssionRuntime = shouldRenderFilterIcon && isFilterPanelOpened && shouldRenderSqlExpressionRuntime
    const isNeedToShowCustomSqlBuilder = shouldRenderFilterIcon && isFilterPanelOpened && isCustomSqlBuilderVisible
    const newIsNeedDataSource = isNeedDataSourceState || isItemNotFromLayer || isNeedToExecuteSelectTask || isNeedToShowSqlExresssionRuntime || isNeedToShowCustomSqlBuilder
    return newIsNeedDataSource
  }, [isCustomSqlBuilderVisible, isFilterPanelOpened, isNeedDataSourceState, jimuLayerViewId, latestSelectVersion, shouldRenderFilterIcon, shouldRenderSqlExpressionRuntime])
  ...
  // 1. Udpate data source if dsInfo.instanceStatus changes.
  ...
  const onDataSourceInfoChange = React.useCallback((preDsInfo: IMDataSourceInfo, dsInfo: IMDataSourceInfo) => {
    ...
    // Udpate data source if dsInfo.instanceStatus changes.
    if (dsInfo) {
  ...
  React.useEffect(() => {
    ...
    const progressCallback = (progress: number) => {
      // only updage progress when current selecting task is fresh
  ...
  React.useEffect(() => {
    const tempIsFilterIconVisible = shouldShowAttributeFilterIcon(isGenerated, configSqlExpression)

    updateDataSourceItemRuntimeInfoForUid(uid, {
      shouldRenderFilterIcon: tempIsFilterIconVisible,
      imDisplaySqlExpression: configSqlExpression
    })
  }, [isGenerated, configSqlExpression, uid, updateDataSourceItemRuntimeInfoForUid])
  ...
  const tryExecuteSelectingByGeometryInfoAndSqlUI = React.useCallback(async (selectionMode: DataSourceSelectionMode, newGeometryInfo: GeometryInfo) => {
    ...
    // getOrCreateDataSource() is an async method, durying this period, isAttributeFilterOn and imDisplaySqlExpression maybe changes,
  ...
  const clearSelection = React.useCallback(async () => {
    ...
    // tryExecuteSelectingByGeometryInfoAndSqlUI(selectionMode, null, '') will update selection with emtpty records
  ...
  const onSqlExpressionChanged = React.useCallback((newImSqlExpression: IMSqlExpression) => {
    const isFilterIconVisible = shouldShowAttributeFilterIcon(isGenerated, newImSqlExpression)

    updateDataSourceItemRuntimeInfoForUid(uid, {
      shouldRenderFilterIcon: isFilterIconVisible,
      imDisplaySqlExpression: newImSqlExpression
    })
  }, [isGenerated, updateDataSourceItemRuntimeInfoForUid, uid])
