...
import { type DataSourceItem, type IMSpatialSelection, UnitType, mapConfigUnitTypeToJSAPIUnit, mapConfigSpattialRelToJSAPISpatialRel, mapConfigSpatialRelToStringKey } from '../../config'
import {
...
export default function SelectByLocation (props: SelectByLocationProps): React.ReactElement {
  ...
  const [isQuryingLocationGeometry, setIsQueryingLocationGeometry] = React.useState<boolean>(false)
  ...
  const isApplyBtnEnabled = React.useMemo(() => {
    ...
    return result
  }, [isQuryingLocationGeometry, lengthOfCheckedReadyRuntimeInfos, lengthOfRuntimeInfosForBatch, relationshipSelectValue, selectedDataSource])
  ...
  const onApplyBtnClicked = React.useCallback(() => {
    ...
    p.then((geometry) => {
      ...
      const apiSpatialRel = mapConfigSpattialRelToJSAPISpatialRel[relationshipSelectValue]
  ...
  const loadedLocationDataSources = React.useMemo(() => {
    ...
    imSpatialSelection.useDataSources.forEach(imDataSource => {
  ...
  }, [dataSourceCache, imSpatialSelection.useDataSources])
  ...
  return (
    <div className={classNames(['select-by-location p-4', { 'd-none': !visible }])} css={style}>
      ...
      <div className='data-view-param w-100 d-flex flex-column align-items-center mb-2'>
        ...
        {
          imSpatialSelection.useDataSources.map(imUseDataSource => {
...
async function getBufferedUnionGeometry (
  ...
  const queryParams: ArcGISQueryParams = {
    outFields: [],
    returnGeometry: true,
    // Must make sure the outSpatialReference is webmercator.
    // If the returned geometriy spatialReference is 4326, then geometryEngineAsync.buffer() will get the following error:
  ...
  if (rawGeometries.length > 0) {
    if (enableBuffer && bufferDistanceInputValue > 0 && bufferUnitSelectValue) {
      ...
      // Must make sure the input geometry spatialReference is webmercator.
