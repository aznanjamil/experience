...
export interface DataSourceItemRuntimeInfo {
  ...
  // true means the data source is generated at runtime, otherwise false
  isGenerated: boolean
...
export function getInitialDataSourceItemRuntimeInfoMap (
  ...
  imDataSourceItems.forEach((imDataSourceItem) => {
    ...
    const isFilterIconVisible = shouldShowAttributeFilterIcon(isGenerated, imSqlExpression)
 ...
 * @param isGenerated true means the data source is generated at runtime.
 * @param imConfigSqlExpression Note, imConfigSqlExpression must be the sqlExpression from config.
 * @returns
 */
export function shouldShowAttributeFilterIcon (isGenerated: boolean, imConfigSqlExpression: IMSqlExpression): boolean {
  let show: boolean = false

  if (isGenerated) {
    // data source is generated at runtime
    show = true
  } else {
    ...
    if (imConfigSqlExpression) {
      ...
      show = (!!getFinalAppliedSql(imConfigSqlExpression.sql)) || shouldShowSqlExpressionRuntime(isGenerated, imConfigSqlExpression)
    }
  }
 ...
 * @returns
 */
export function shouldShowSqlExpressionRuntime (isGenerated: boolean, imConfigSqlExpression: IMSqlExpression): boolean {
  let show: boolean = false

  // only show SqlExpressionRuntime for not generated data sources because we show SqlExpressionBuilder for generated data sources
  if (!isGenerated && imConfigSqlExpression) {
