...
export default function SpatialSection (props: SpatialSectionProps): React.ReactElement {
  ...
  const onTitleWithSwitchChange = React.useCallback((evt, checked) => {
    const newImSpatialSelection = imSpatialSelection.set('enable', checked)
    ...
    if (addedDefaultViewDsIds.length > 0) {
      ...
      finalNewUseDataSources = evtUseDataSources.map(useDataSource => {
        if (addedDefaultViewDsIds.includes(useDataSource.dataSourceId)) {
          // useDataSource is the new added data source, it is deafault view, need to change it to selection view by default
  ...
  const onRelationshipMultiSelectClickItem = React.useCallback((evt, value, selectedValues: string[]) => {
    if (selectedValues.length === 0) {
      // Make sure have one reletionship at least.
      return
    }
    ...
    >
      ...
      >
        <TitleWithSwitch
          ...
          enabled={enableSpatialSelection}
