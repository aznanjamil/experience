...
*/
export const getPortaUrlByUtility = async (utility: IMUseUtility): Promise<string> => {
...
*/
export const getPortaUrlByServiceUrl = async (serviceUrl: string): Promise<string> => {
...
export const getSessionByUtility = async (utility: IMUseUtility): Promise<any> => {
  const portalUrl = await getPortaUrlByUtility(utility)
...
async function initLayoutTemplateOfNewTemplate (config: IMConfig, temp: PrintTemplateProperties): Promise<PrintTemplateProperties> {
  ...
  if (supportCustomLayout && defaultCustomLayoutItem) {
    //Has default custom lyoput template
    const params = {
