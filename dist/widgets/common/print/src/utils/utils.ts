...
import { getPortaUrlByServiceUrl } from './service-util'
import {
...
export async function getTemplateOrReportInfo (printServiceUrl: string, itemInfoType?: ItemInfoType, itemId?: string): Promise<any[]> {
  ...
  const portalUrl = await getPortaUrlByServiceUrl(printServiceUrl)
...
export function checkIsUtilityAvaible (utilityId: string, appRuntimeInfo: ImmutableObject<AppRuntimeInfo>): boolean {
