...
const CustomSetting = (props: Props) => {
  ...
  const initPreAvailableReport = (template: IMPrintTemplateProperties) => {
    ...
    const isReportTemplateAvaible = checkIsReportsTemplateAvailable(option)
    if (isReportTemplateAvaible) {
  ...
  const checkAndResetReportTemplate = hooks.useEventCallback((templateItem: PrintTemplateProperties): TemplateItemUpdateTypes => {
    ...
    if (!isReportTemplateAvaible && preAvailableReportRef.current && preAvailableReportRef.current?.templateId === templateItem.templateId) {
