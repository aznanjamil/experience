...
import { getNewConfigByCustomReportItem, getPortaUrlByUtility } from '../../utils/service-util'
...
const ReportTemplateSetting = (props: Props) => {
  ...
  useEffect(() => {
    getPortaUrlByUtility(config.useUtility).then(portalUrl => {
  ...
  const handleDsChange = (useDataSources: UseDataSource[], key: string) => {
    ...
    udpateWidgetJson.config = newConfig
    udpateWidgetJson.useDataSources = newUseDataSources
  ...
  const getReportSectionOverridesSetting = () => {
    ...
    const reportSectionOverridesLegnth = Object.keys(reportSectionOverrides)?.length
    if (reportSectionOverridesLegnth === 0) return
